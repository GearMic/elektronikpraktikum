...
def plot_data(data_list, filename):
    ...
    orange = mlines.Line2D([], [], color='orange', marker='o',linestyle='None', label=r'$v=$'+str(verstärkung[0])+r', $C=0.1\mu F$')
    ...
    line = mlines.Line2D([], [], color='gray', linestyle='-', label=r'$v_\text{max}$')
    dashed_line = mlines.Line2D([], [], color='gray', linestyle='--', label=r'$v_\text{grenz}=v_\text{max}-3\text{dB}$')
    dotted_line = mlines.Line2D([], [], color='gray', linestyle=':', label='Regression')

    ax.legend(handles=[blue,red,orange,green,marker,line,dashed_line,dotted_line],fontsize='small',loc='upper left')
    plt.tight_layout()
